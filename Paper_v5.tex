...
Meta-heuristic optimization approaches like Genetic Algorithms (GAs) \cite{goldberg2006genetic}, Simulated Annealing (SA) \cite{kirkpatrick1983optimization},
Evolutionary Programming \cite{fogel2009artificial} and Differential Evolution (DE) \cite{storn1997differential,tsai2005evolutionary,wang2001hybrid,noman2007inferring} have shown promise on non-linear multi-modal problems \cite{sun2012parameter}.
These techniques do not make any assumptions, nor do they require, \textit{a~priori} information about the structure of objective function.
...
parameters in models with expensive function evaluations \cite{tolson2007dynamically}. This approach called Dynamically Dimensioned Search (DDS) is a simple stochastic single-solution based heuristic that estimates nearly optimal solutions within a specified maximum function (or model) evaluation limit.
Taken together, meta-heuristics are often very effective at finding globally optimal or near optimal solutions, however the large number of function evaluations required to
converge to a solution limits their application for expensive biochemical models.
...
In this study, we developed Dynamic Optimization with Particle Swarms (DOPS), a novel meta-heuristic that combines the global search capability of multi-swarm particle swarm optimization with the greedy refinement of dynamically dimensioned search (DDS).
