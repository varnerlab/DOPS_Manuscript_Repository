...
Mathematical modeling has evolved as a powerful paradigm to analyze, and ultimately design these complex networks \cite{assmus2006dynamics, Riel2006aa,Jaqaman:2006aa,kitano2002systems,hood2004systems}. Mathematical modeling of biochemical networks is often an iterative process.
...
Optimal parameter estimates obtained from model training are then used to validate the model on unseen experimental data.
...
the complex dynamics of large biological systems and noisy, often incomplete experimental data sets pose a unique challenge.
...
Non-linearity coupled with multi-modality generally renders local optimization techniques such as pattern search \cite{hooke1961direct}, Nelder-Mead simplex methods \cite{nelder1965simplex}, steepest descent methods or Levenberg-Marquardt \cite{more1978levenberg} incapable of reliably obtaining globally optimal solutions as these methods often terminate at local minimum.
...
the absence of derivative information, the discontinuity of the objective functions, non-smooth regions or the lack of knowledge about the objective function hampers these techniques.
...
These techniques do not make any assumptions about, nor do they require, \textit{a~priori} information about the structure of objective function.
Though they do not guarantee strong convergence, meta-heuristic approaches are often effective in finding globally optimal or near optimal solutions.
For example, Mendes et al. used simulated annealing to estimate rate constants for the irreversible inhibition of HIV proteinase \cite{mendes1998non},
while Modchang et al. used a genetic algorithm (GA) to estimate parameters for a model of G-protein-coupled receptors (GPCRs) activity \cite{modchang2008mathematical}. Parameter estimates obtained using GA helped qualitatively determine the effectiveness of two G-protein agonists, N6-cyclopentyladenosine (CPA) and 5'-N-ethylcarboxamidoadenosine (NECA).
Differential evolution (DE) have also been effective on various biological problems \cite{tsai2005evolutionary,wang2001hybrid,noman2007inferring}.
For example, Tashkova et al. compared different meta-heuristics for parameter estimation on a dynamic model of endocytosis
and showed that DE was the most effective of the approaches tested \cite{tashkova2011parameter}.
...
Hybrid approaches that combine a meta-heuristic with a local optimization search,
where a near optimal solution estimated using a meta-heuristic is refined using a local search, have also become popular.
For example, Villaverde et al. developed the enhanced scatter search (eSS) method, which combined scatter search with local search methods,
for parameter estimation in a collection of systems biology models \cite{villaverde2015biopredyn}.
However, despite these successes, a major drawback of most meta-heuristics is the large number of function evaluations required to explore parameter space.
Performing a large number of potentially expensive function evaluations is not desirable (and perhaps not feasible) for many types of biochemical models.
...
In many high dimensional problems approaching an exact solution may not be necessary. Gutenkust et al. showed that a number of systems biology models are 'sloppy' \cite{gutenkunst2007universally}. Sloppy systems have specific parameter combinations that largely define the dynamics of the system. Large perturbations to the rest of the parameters does not greatly impact the system dynamics. Ensemble approaches \cite{song2010ensembles,luan2010ensembles} have exploited this aspect to describe the dynamics of biological systems including coagulation which can be described using only a set of key species or parameters \cite{sagar2015dynamic}. Tolson and Shoemaker \cite{tolson2007dynamically} showed through Dynamically Dimensioned Search (DDS) that high-dimensional watershed models can be calibrated quickly by perturbing only a subset of dimensions. DDS is a single solution based search where the solution is greedily updated. Because the neighborhood in DDS is the entire parameter space, DDS is a global optimization technique although it is greedy.

In this study, we developed Dynamic Optimization with Particle Swarms (DOPS), a novel meta-heuristic that combines the global search capability of multi-swarm particle swarm optimization with the greedy refinement of dynamically dimensioned search.
...
DOPS uses multi-swarm particle swarm optimization to generate candidate solutions, which are then greedily updated using dynamically dimensioned search.
First, we tested the performance of DOPS on commonly used test functions for global optimization (Ackley and Rosenbrock) and published biochemical benchmark problems. Next, we used DOPS to identify a model of human coagulation cascade.
DOPS outperformed common meta-heuristic approaches like differential evolution, simulated annealing, and dynamically dimensioned search on the test functions, benchmark problems and the coagulation model. For example, DOPS recovered the nominal parameters for the benchmark problems using an order of magnitude less function evaluations than eSS in all cases. Taken together, these studies suggest DOPS is a promising meta-heuristic approach for the estimation of biochemical model parameters in relatively few function evaluations.
