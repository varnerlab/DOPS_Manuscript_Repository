function[]=runForTable1(lowerIter, upperIter)
    ...
    addpath(genpath('testFunctions')) %add the test functions to path
    ...
    for j=1:size(functions_to_test,2) %this can be changed based on which functions you wish to test
        load(bounds_files{j});
