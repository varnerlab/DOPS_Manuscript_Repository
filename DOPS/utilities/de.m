...
val(1)  = feval (fcn, [pop(ibest,:)' const]); %Passing transpose - Adi
...
while ((iter < maxiter) && (nfeval < maxnfe) &&  (bestval > VTR)  && ...
  ...
  elseif (st == 5)                  	  % DE/rand/2
   ...
   % ui=bind(ui,XVmin,XVmax);	
    ...
    o = ones (NP, 1);
    wi = sort ([0.05*o, wi, 0.95*o],2); % sort and take the second column
    wi=wi(:,2);    
  ...
  bmem(iter,:)=bestmem';                           % strategies.

  %% ---- Output section ----------------------------------------------------

  if (refresh > 0)
    if (rem (iter, refresh) == 0)
       fprintf ('Iteration: %d,  Best: %8.4e,  Worst: %8.4e\n', ...
